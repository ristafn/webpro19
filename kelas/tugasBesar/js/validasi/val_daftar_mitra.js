...
function errorTeleponFun(){
    errorTelepon.style.display = 'block'

    if (telepon.value.length >= 10 && telepon.value.length <= 12) {
        errorTelepon.classList.remove("text-danger")
        errorTelepon.classList.add("text-success")
    } else {
        errorTelepon.classList.remove("text-success")
        errorTelepon.classList.add("text-danger")
    }
}
